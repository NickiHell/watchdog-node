...
class SLAMMapper:
    ...
    def __init__(self, node: Node):
        ...
        self.map_sub = node.create_subscription(OccupancyGrid, "/map", self.map_callback, 10)

        self.map_metadata_sub = node.create_subscription(MapMetaData, "/map_metadata", self.map_metadata_callback, 10)

        self.pose_sub = node.create_subscription(PoseWithCovarianceStamped, "/pose", self.pose_callback, 10)
    ...
    def initial_pose_callback(self, msg: PoseWithCovarianceStamped):
        ...
        """
        self.logger.info(
            f"Установлена начальная позиция: x={msg.pose.pose.position.x:.2f}, y={msg.pose.pose.position.y:.2f}"
        )
    ...
    def get_pose(self) -> PoseWithCovarianceStamped | None:
    ...
    def get_robot_position(self) -> tuple | None:
    ...
    def get_map_data_as_array(self) -> np.ndarray | None:
