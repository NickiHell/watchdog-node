...
class ObstacleAvoidance:
    ...
    def compute_safe_velocity(self, desired_linear: float, desired_angular: float) -> Tuple[float, float]:
        ...
        if front_obstacle:
            ...
            if distance < self.safety_distance:
                # Очень близко - останавливаемся и поворачиваем
                safe_linear = 0.0
                safe_angular = self._compute_avoidance_angle(angle)
            else:
                # Замедляемся
                safe_linear = desired_linear * (distance / self.safety_distance)
    ...
    def _check_front_obstacle(self) -> Optional[Tuple[float, float]]:
        ...
        valid_mask = (
            (ranges >= self.last_scan.range_min) &
            (ranges <= self.last_scan.range_max) &
            (np.isfinite(ranges))
        )
    ...
    def check_clear_path(self, direction_angle: float = 0.0, width: float = 0.3) -> bool:
        ...
        sector_mask = (
            (valid_angles >= direction_angle - width / 2.0) &
            (valid_angles <= direction_angle + width / 2.0)
        )
    ...
    def get_safe_direction(self) -> Optional[float]:
        ...
        for i in range(sector_count):
            ...
            sector_mask = (
                (valid_angles >= sector_start) &
                (valid_angles < sector_end)
            )
